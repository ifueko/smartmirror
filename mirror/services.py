...
class MCPService:
    ...
    async def connect(self):
        if not self.client.session: # Or use a more robust self.is_connected flag
            server_script_path = os.path.join(settings.BASE_DIR, 'mirror', 'mirrordb_server.py')
            ...
            try:
                ...
                logger.error(f"Failed to connect MCP client during startup: {e}", exc_info=True)
                self.is_connected = False
    ...
    async def process_query(self, query: str) -> str:
        if not self.client.session or not self.is_connected:
            ...
            raise Exception("MCP Client is not connected. Please ensure the service is running.")
...
async def get_mcp_service():
    ...
    if not service.is_connected and not service.client.session:
        ...
        try:
            ...
            logger.error(f"On-demand connection failed for MCPService: {e}", exc_info=True)
