...
class ASRConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        if self.full_audio_buffer and not self.transcription_started:
            ...
            await self.transcribe_and_send(bytes(self.full_audio_buffer), reason="disconnect_flush")

        attributes_to_delete = ['client_sample_rate', 'resampler_to_16k',
                                    'full_audio_buffer', 'transcription_started']
    ...
    async def transcribe_and_send(self, pcm_audio_bytes, reason=""):
        if not pcm_audio_bytes:
            ...
            return True
        logger.info(f"Transcribing full audio ({len(pcm_audio_bytes)} bytes) due to: {reason}")
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if text_data:
            try:
                ...
                if message.get("type") == "audio_config":
                    ...
                    logger.info("Received end_stream signal from client. Starting transcription.")
                    self.transcription_started = True
                    await self.transcribe_and_send(bytes(self.full_audio_buffer), reason="end_of_stream")
        ...
        if bytes_data:
            ...
            logger.info(f"Received audio chunk ({len(bytes_data)} bytes), total buffer size: {len(self.full_audio_buffer)} bytes.")
